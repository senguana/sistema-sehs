...
$(document).ready(function() {
		...
		$("#NuevoFacturaAjax").submit(function() {
			var form = $(this);

			$('.form-group').removeClass('has-error').removeClass('has-success');
			$('.text-danger').remove();
			var codigo_colportor = $('#codigo_colportor').val();
			var paid = $("#paid").val();
	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'</td>'+
				'<td>'+
					'<button class="btn btn-danger btn-sm removeProductRowBtn" type="button" onclick="removeProductRow('+count+')"><i class="glyphicon glyphicon-trash"></i></button>'+
				'</td>'+
			'</tr>';
			if(tableLength > 0) {							
	...
	if(row) {
		...
		if(productId == "") {
			...
			$.ajax({
				url: 'ajax/libroAjax.php',
				type: 'post',
				data: {libroId : productId},
				dataType: 'json',
				success:function(response) {
					// setting the rate value into the rate input field
					
					$("#rate"+row).val(response.precioLibro);
					$("#rateValue"+row).val(response.precioLibro);

					$("#quantity"+row).val(1);

					var total = Number(response.precioLibro) * 1;
...
function subAmount() {
	...
	// vat
	var vat = (Number($("#subTotal").val())/10);
	vat = vat.toFixed(2);
	$("#vat").val(vat);
	$("#vatValue").val(vat);
